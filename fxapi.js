...
function _typeof(o) {
    ...
    function setServerCountry(url, country) {
        ...
        var normalized = url.replace(/^https?:\/\//, "").replace(/\/+$/, "").toLowerCase();
    ...
    var Config = {
        getHostKey: function getHostKey() {
            return _getHostKey()
        },
        Urls: {
            ...
            getLampOnline: function getLampOnline() {
                return getServerUrl()
            },
    ...
    function ensureBalansersWithSearch() {
        ...
        balansers_with_search_promise = new Promise(function(resolve) {
            ...
            net.timeout(10000);
            net.silent(account(Defined.getLocalhost() + "lite/withsearch"), function(json) {
            ...
            window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
                ...
                if (!window.rch_nws[hk].startTypeInvoke) {
                    ...
                    if (Lampa.Platform.is("android") || Lampa.Platform.is("tizen")) check(true);
                        ...
                        var net = new Lampa.Reguest;
                        net.silent(Config.Urls.getLampOnline().indexOf(location.host) >= 0 ? Config.Urls.GithubCheck : host + Config.Urls.CorsCheckPath, function() {
                            check(true)
                        }, function() {
                            check(false)
                        }, false, {
    ...
    if (getActiveHostKey()) {
        window.rch_nws[getActiveHostKey()].typeInvoke(Config.Urls.getLampOnline(), function() {})
    }
    var domParser = null;
    ...
    function getRealKpId(imdbId, callback) {
        if (!imdbId) {
            callback(null);
            return
        }
        if (realKpIdCache[imdbId]) {
            callback(realKpIdCache[imdbId]);
            return
        }
        var net = new Lampa.Reguest;
        net.timeout(5000);
        net.silent("https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-filters?imdbId=" + imdbId, function(json) {
            if (json && json.films && json.films.length > 0) {
                var kpId = json.films[0].filmId;
                realKpIdCache[imdbId] = kpId;
                callback(kpId)
            } else {
                callback(null)
            }
    ...
    function isDirectFxapi() {
        var serverUrl = getServerUrl();
        return serverUrl.indexOf("/lite/fxapi") !== -1
    }

    function getServerBase() {
        var serverUrl = getServerUrl();
        var idx = serverUrl.indexOf("/lite/fxapi");
        if (idx !== -1) {
    ...
    function getFxapiCachedValue(key, generator) {
        var data = Lampa.Storage.get(key, null);
        var now = Date.now();
        if (data && data.value && data.expires && data.expires > now) {
            return data.value
        }
        var newValue = generator();
        Lampa.Storage.set(key, {
            value: newValue,
            expires: now + FXAPI_CACHE_TTL
        });
        return newValue
    }
    ...
    function getRandomUid() {
        if (isDirectFxapi()) {
            return getFxapiCachedValue("lamponline_fxapi_uid", generateRandomUid)
        }
        var uid = Lampa.Storage.get("lamponline_uid", "");
        if (!uid) {
            uid = generateRandomUid();
            Lampa.Storage.set("lamponline_uid", uid)
        }
        return uid
    }
    ...
    function getAuthToken() {
        if (isDirectFxapi()) {
            return getFxapiCachedValue("lamponline_fxapi_token", generateAuthToken)
        }
        var token = Lampa.Storage.get("lamponline_token", "");
        if (!token) {
            token = generateAuthToken();
            Lampa.Storage.set("lamponline_token", token)
        }
        return token
    }
    ...
    function buildUrl(url, query) {
        ...
        var serverUrl = getServerUrl();
        var isFxapi = isDirectFxapi();

        // --- ДОБАВЛЕНО ДЛЯ FILMIX ---
        if (url.indexOf("/lite/filmix") !== -1) {
            if (url.indexOf("account_email=") == -1) {
                url = Lampa.Utils.addUrlComponent(url, "account_email=" + encodeURIComponent("aklama@mail.ru"))
            }
        ...
        if (url.indexOf("uid=") == -1) {
            url = Lampa.Utils.addUrlComponent(url, "uid=" + encodeURIComponent(getRandomUid()))
        }
        if (isFxapi && url.indexOf("showy_token=") == -1) {
            url = Lampa.Utils.addUrlComponent(url, "showy_token=" + encodeURIComponent(getAuthToken()))
        }
        return url
    }
    ...
    function component(object) {
        ...
        var NetworkManager = function() {
            ...
            function buildMovieUrl(url) {
                ...
                // --- ИЗМЕНЕНО ДЛЯ FILMIX ---
                var isFilmix = serverUrl.indexOf("/lite/filmix") !== -1;

                if (isDirectFxapi() || isFilmix) {
                    // --- КОНЕЦ ИЗМЕНЕНИЯ ---

                    var query = [];
                    query.push("rjson=False");
                    var title = object.clarification ? object.search : object.movie.title || object.movie.name;
                    query.push("title=" + encodeURIComponent(title));
                    query.push("s=" + (object.clarification ? 0 : 1));
                ...
                if (type == "filter") {
                    ...
                    if (a.add) {
                        openServerInput(function(new_value) {
                            if (new_value) {
                                serverBtn.find("div").text(getServerUrl() ? formatServerDisplay(getServerUrl()) : Lampa.Lang.translate("lampac_not_set"));
                                Lampa.Activity.replace()
                            }
                        })
                    } else {
        ...
        this.externalids = function() {
            ...
            if (object.movie.imdb_id) query.push("imdb_id=" + (object.movie.imdb_id || ""));
            if (object.movie.kinopoisk_id) query.push("kinopoisk_id=" + (object.movie.kinopoisk_id || ""));
            var url = account(Defined.getLocalhost() + "externalids?" + query.join("&"));
        ...
        this.lifeSource = function() {
            var _this3 = this;
            return new Promise(function(resolve, reject) {
                var url = _this3.requestParams(Defined.getLocalhost() + "lifeevents?memkey=" + (_this3.memkey || ""));
                var resolved = false;
                var stopped = false;
        ...
        this.createSource = function() {
            ...
            if (isDirectFxapi()) {
                ...
                return Promise.resolve(_this4.startSource(fxapiSource))
            }
            var url = _this4.requestParams(Defined.getLocalhost() + "lite/events?life=true");
        ...
        this.request = function(url) {
            ...
            if (isDirectFxapi()) {
                ...
                var domain = base.replace(/^https?:\/\//, "").replace(/\/.*$/, "");
                var regex = new RegExp(domain.replace(/\./g, "\\.") + "([^/])");
                url = url.replace(regex, domain + "/$1")
            }
                ...
                if (elem.img) {
                    ...
                    if (elem.img !== undefined) {
                        if (elem.img.charAt(0) === "/") {
                            var baseUrl = isDirectFxapi() ? getServerBase() : Defined.getLocalhost();
                            elem.img = baseUrl + elem.img.substring(1)
                        }
                    ...
                    tempImg.onload = function() {
                        if (destroyed) return;
                        img.src = tempImg.src;
                        ifető(" " + img.className + " ").indexOf(" loaded ") == -1) img.className = (img.className ? img.className + " " : "") + "loaded"
                    };
        ...
        this.filter = function(filter_items, choice) {
            ...
            filter.chosen("server", [getServerUrl() ? formatServerDisplay(getServerUrl()) : Lampa.Lang.translate("lampac_not_set")]);
    ...
    function addSourceSearch(spiderName, spiderUri) {
        ...
        var source = {
            ...
            search: function search(params, oncomplite) {
                                    ...
                                    var cards = line.data.map(function(item) {
                                        ...
                                        if (item.img !== undefined) {
                                            if (item.img.charAt(0) === "/") {
                                                var baseUrl = isDirectFxapi() ? getServerBase() : Defined.getLocalhost();
                                                item.img = baseUrl + item.img.substring(1)
                                            }
                ...
                network.silent(account(Defined.getLocalhost() + "lite/" + spiderUri + "?title=" + params.query), function(json) {
                    if (json.rch) {
                        rchRun(json, function() {
                            network.silent(account(Defined.getLocalhost() + "lite/" + spiderUri + "?title=" + params.query), function(links) {
            ...
            if (event.name == "main") {
                if (Lampa.Settings.main().render().find("[data-component=\"lamponline_settings\"]").length == 0) {
                    Lampa.SettingsApi.addComponent({
                        ...
                        icon: "<svg viewBox=\"0 0 16 16\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M11.783 10.094c-1.699.998-3.766 1.684-5.678 1.95a1.66 1.66 0 0 1-.684.934c.512 1.093 1.249 2.087 2.139 2.987a7.98 7.98 0 0 0 6.702-3.074l.083-.119c-.244-.914-.648-1.784-1.145-2.644q-.134.038-.261.062c-.143.04-.291.068-.446.068a1.7 1.7 0 0 1-.71-.164M9.051 5.492a18 18 0 0 0-2.004-1.256 1.67 1.67 0 0 1-1.907.985c-.407 1.535-.624 3.162-.511 4.694a1.67 1.67 0 0 1 1.52 1.354c1.695-.279 3.47-.879 4.967-1.738a1.67 1.67 0 0 1-.297-.949c0-.413.156-.786.403-1.078-.654-.736-1.389-1.443-2.171-2.012M4 9.989c-.137-1.634.104-3.392.541-5.032a1.67 1.67 0 0 1-.713-1.369c0-.197.039-.386.104-.562a18 18 0 0 0-1.974-.247c-.089.104-.185.204-.269.314a7.98 7.98 0 0 0-1.23 7.547 9.5 9.5 0 0 0 2.397.666A1.67 1.67 0 0 1 4 9.989m9.928-.3c-.029.037-.064.067-.096.1.433.736.799 1.482 1.053 2.268a7.98 7.98 0 0 0 .832-6.122c-.09.133-.176.267-.271.396-.436.601-.875 1.217-1.354 1.772.045.152.076.311.076.479v.004c.084.374.013.779-.24 1.103M7.164 3.447c.799.414 1.584.898 2.33 1.44.84.611 1.627 1.373 2.324 2.164.207-.092.434-.145.676-.145.5 0 .945.225 1.252.572.404-.492.783-1.022 1.161-1.54.194-.268.372-.543.544-.82A7.96 7.96 0 0 0 7.701.012q-.173.217-.339.439c-.401.552-.739 1.08-1.04 1.637.039.029.064.066.1.1.417.276.697.734.742 1.259m-4.285 8.518a10 10 0 0 1-2.07-.487 7.95 7.95 0 0 0 5.806 4.397 11 11 0 0 1-1.753-2.66 1.675 1.675 0 0 1-1.983-1.25m1.635-9.723a1.32 1.32 0 0 1 1.199-.416C6.025 1.24 6.377.683 6.794.104a7.97 7.97 0 0 0-4.247 2.062c.59.066 1.176.14 1.761.252q.096-.095.206-.176\" fill=\"currentColor\"/>\n</svg>\n\n        <span>#{title_online}</span>\n    </div>");Lampa.Component.add("lamponline",component);function addButton(e){if(e.render.find(".lampac--button").length)return;var btn=$(Lampa.Lang.translate(button));btn.on("hover:enter",function(){Lampa.Component.add("lamponline",component);var id=Lampa.Utils.hash(e.movie.number_of_seasons?e.movie.original_name:e.movie.original_title);var all=Lampa.Storage.get(Config.StorageKeys.ClarificationSearch,"{}");Lampa.Activity.push({url:"",title:Lampa.Lang.translate("title_online"),component:"lamponline",search:all[id]?all[id]:e.movie.title,search_one:e.movie.title,search_two:e.movie.original_title,movie:e.movie,page:1,clarification:all[id]?true:false})});e.render.before(btn)}Lampa.Listener.follow("full",function(e){if(e.type=="complite"){addButton({render:e.object.activity.render().find(".view--torrent"),movie:e.data.movie})}});try{if(Lampa.Activity.active().component=="full"){addButton({render:Lampa.Activity.active().activity.render().find(".view--torrent"),movie:Lampa.Activity.active().card})}}catch(e){console.error(e)}if(Lampa.Manifest.app_digital>=177){var balansers_sync=["filmix","filmixtv","fxapi","rezka","rhsprem","lumex","videodb","collaps","collaps-dash","hdvb","zetflix","kodik","ashdi","kinoukr","kinotochka","remux","iframevideo","cdnmovies","anilibria","animedia","animego","animevost","animebesst","redheadsound","alloha","animelib","moonanime","kinopub","vibix","vdbmovies","fancdn","cdnvideohub","vokino","rc/filmix","rc/fxapi","rc/rhs","vcdn","videocdn","mirage","hydraflix","videasy","vidsrc","movpi","vidlink","twoembed","autoembed","smashystream","autoembed","rgshows","pidtor","videoseed","iptvonline","veoveo"];balansers_sync.forEach(function(name){Lampa.Storage.sync(Config.StorageKeys.OnlineChoicePrefix+name,"object_object")});Lampa.Storage.sync(Config.StorageKeys.OnlineWatchedLast,"object_object")}Lampa.Settings.listener.follow("open",function(event){if(event.name=="main"){if(Lampa.Settings.main().render().find("[data-component=\"lamponline_settings\"]").length==0){Lampa.SettingsApi.addComponent({component:"lamponline_settings",name:Lampa.Lang.translate("lampac_settings_title"),icon:"<svg viewBox=\"0 0 16 16\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M11.783 10.094c-1.699.998-3.766 1.684-5.678 1.95a1.66 1.66 0 0 1-.684.934c.512 1.093 1.249 2.087 2.139 2.987a7.98 7.98 0 0 0 6.702-3.074l.083-.119c-.244-.914-.648-1.784-1.145-2.644q-.134.038-.261.062c-.143.04-.291.068-.446.068a1.7 1.7 0 0 1-.71-.164M9.051 5.492a18 18 0 0 0-2.004-1.256 1.67 1.67 0 0 1-1.907.985c-.407 1.535-.624 3.162-.511 4.694a1.67 1.67 0 0 1 1.52 1.354c1.695-.279 3.47-.879 4.967-1.738a1.67 1.67 0 0 1-.297-.949c0-.413.156-.786.403-1.078-.654-.736-1.389-1.443-2.171-2.012M4 9.989c-.137-1.634.104-3.392.541-5.032a1.67 1.67 0 0 1-.713-1.369c0-.197.039-.386.104-.562a18 18 0 0 0-1.974-.247c-.089.104-.185.204-.269.314a7.98 7.98 0 0 0-1.23 7.547 9.5 9.5 0 0 0 2.397.666A1.67 1.67 0 0 1 4 9.989m9.928-.3c-.029.037-.064.067-.096.1.433.736.799 1.482 1.053 2.268a7.98 7.98 0 0 0 .832-6.122c-.09.133-.176.267-.271.396-.436.601-.875 1.217-1.354 1.772.045.152.076.311.076.479v.004c.084.374.013.779-.24 1.103M7.164 3.447c.799.414 1.584.898 2.33 1.44.84.611 1.627 1.373 2.324 2.164.207-.092.434-.145.676-.145.5 0 .945.225 1.252.572.404-.492.783-1.022 1.161-1.54.194-.268.372-.543.544-.82A7.96 7.96 0 0 0 7.701.012q-.173.217-.339.439c-.401.552-.739 1.08-1.04 1.637.039.029.064.066.1.1.417.276.697.734.742 1.259m-4.285 8.518a10 10 0 0 1-2.07-.487 7.95 7.95 0 0 0 5.806 4.397 11 11 0 0 1-1.753-2.66 1.675 1.675 0 0 1-1.983-1.25m1.635-9.723a1.32 1.32 0 0 1 1.199-.416C6.025 1.24 6.377.683 6.794.104a7.97 7.97 0 0 0-4.247 2.062c.59.066 1.176.14 1.761.252q.096-.095.206-.176\" fill=\"currentColor\"/></svg>"})}Lampa.Settings.main().update()}});Lampa.Params.select(STORAGE_KEY_SERVER,"","");Lampa.SettingsApi.addParam({component:"lamponline_settings",param:{name:"lamponline_add_server_btn",type:"static"},field:{name:Lampa.Lang.translate("lampac_add_server"),description:Lampa.Lang.translate("lampac_server_address_desc")},onRender:function onRender(item){item.on("hover:enter",function(){Lampa.Input.edit({title:Lampa.Lang.translate("lampac_server_address"),value:"",placeholder:"192.168.1.1:9118",nosave:true,free:true,nomic:true},function(new_value){if(new_value&&addServer(new_value)){var servers=getServersList();setActiveServerIndex(servers.length-1);ensureRchNws();Lampa.Settings.update()}})})}});Lampa.SettingsApi.addParam({component:"lamponline_settings",param:{name:"lamponline_load_public_servers_btn",type:"static"},field:{name:Lampa.Lang.translate("lampac_load_public_servers"),description:Lampa.Lang.translate("lampac_load_public_servers_desc")},onRender:function onRender(item){item.on("hover:enter",function(){loadPublicServers()})}});Lampa.SettingsApi.addParam({component:"lamponline_settings",param:{name:"lamponline_servers_title",type:"title"},field:{name:Lampa.Lang.translate("lampac_servers_list")}});Lampa.Settings.listener.follow("open",function(event){if(event.name=="lamponline_settings"){renderServersList(event.body)}})}function renderServersList(body){body.find(".lamponline-server-item").remove();var servers=getServersList();var activeIndex=getActiveServerIndex();var titleElem=body.find(".settings-param-title").last();if(!titleElem.length){titleElem=body.find(".settings-param").last()}servers.forEach(function(server,index){var displayName=formatServerDisplay(server);var isActive=index===activeIndex;var item=$("<div class=\"settings-param selector lamponline-server-item\" data-server-index=\""+index+"\">"+"<div class=\"settings-param__name\">"+displayName+"</div>"+"<div class=\"settings-param__value\"></div>"+(isActive?"<div class=\"settings-param__descr\">\u0422\u0435\u043A\u0443\u0449\u0438\u0439 \u0441\u0435\u0440\u0432\u0435\u0440</div>":"")+"</div>");item.on("hover:enter",function(){showServerActions(index,displayName,function(){renderServersList(body)})});titleElem.after(item);titleElem=item});if(servers.length===0){var emptyItem=$("<div class=\"settings-param lamponline-server-item\">"+"<div class=\"settings-param__name\" style=\"opacity: 0.5\">"+Lampa.Lang.translate("lampac_not_set")+"</div>"+"</div>");titleElem.after(emptyItem)}Lampa.Controller.collectionSet(body);Lampa.Controller.collectionFocus(false,body)}function editServer(index,newUrl){var servers=getServersList();if(index>=0&&index<servers.length&&newUrl){var oldUrl=servers[index];servers[index]=newUrl;Lampa.Storage.set(STORAGE_KEY_SERVERS,servers);var oldServer=Lampa.Storage.get(STORAGE_KEY_SERVER,"");if(oldServer===oldUrl){Lampa.Storage.set(STORAGE_KEY_SERVER,newUrl)}return true}return false}function showServerActions(index,serverName,callback){var servers=getServersList();var activeIndex=getActiveServerIndex();var isActive=index===activeIndex;var items=[];if(!isActive){items.push({title:Lampa.Lang.translate("lampac_select_this"),select:true})}items.push({title:Lampa.Lang.translate("lampac_edit_server"),edit:true});items.push({title:Lampa.Lang.translate("lampac_delete_server"),remove:true});var enabled=Lampa.Controller.enabled().name;Lampa.Select.show({title:serverName,items:items,onBack:function onBack(){Lampa.Controller.toggle(enabled)},onSelect:function onSelect(item){if(item.select){setActiveServerIndex(index);ensureRchNws();Lampa.Controller.toggle(enabled);if(callback)callback()}else if(item.edit){Lampa.Input.edit({title:Lampa.Lang.translate("lampac_server_address"),value:servers[index],placeholder:"192.168.1.1:9118",nosave:true,free:true,nomic:true},function(new_value){if(new_value&&new_value!==servers[index]){editServer(index,new_value);ensureRchNws()}Lampa.Controller.toggle(enabled);if(callback)callback()})}else if(item.remove){removeServer(index);Lampa.Controller.toggle(enabled);if(callback)callback()}}})}function openServerInput(callback){Lampa.Input.edit({title:Lampa.Lang.translate("lampac_server_address"),value:"",placeholder:"192.168.1.1:9118",nosave:true,free:true,nomic:true},function(new_value){if(new_value){addServer(new_value);var servers=getServersList();setActiveServerIndex(servers.length-1);ensureRchNws()}if(callback)callback(new_value);else if(new_value){Lampa.Activity.replace()}})}function openServerSelect(callback){var servers=getServersList();var activeIndex=getActiveServerIndex();var items=[];servers.forEach(function(server,index){items.push({title:formatServerDisplay(server),index:index,selected:index===activeIndex})});items.push({title:Lampa.Lang.translate("lampac_add_server"),add:true});var enabled=Lampa.Controller.enabled().name;Lampa.Select.show({title:Lampa.Lang.translate("lampac_select_server"),items:items,onBack:function onBack(){Lampa.Controller.toggle(enabled)},onSelect:function onSelect(item){if(item.add){openServerInput(function(new_value){if(callback)callback();Lampa.Controller.toggle(enabled);if(new_value)Lampa.Activity.replace()})}else if(item.selected){var displayName=formatServerDisplay(servers[item.index]);Lampa.Select.show({title:displayName,items:[{title:Lampa.Lang.translate("lampac_edit_server"),edit:true},{title:Lampa.Lang.translate("lampac_delete_server"),remove:true}],onBack:function onBack(){Lampa.Controller.toggle(enabled);if(callback)callback()},onSelect:function onSelect(a){if(a.edit){Lampa.Input.edit({title:Lampa.Lang.translate("lampac_server_address"),value:servers[item.index],placeholder:"192.168.1.1:9118",nosave:true,free:true,nomic:true},function(new_value){if(new_value&&new_value!==servers[item.index]){editServer(item.index,new_value);ensureRchNws()}Lampa.Controller.toggle(enabled);if(callback)callback()})}else if(a.remove){removeServer(item.index);if(callback)callback();Lampa.Controller.toggle(enabled);Lampa.Activity.replace()}}})}else{setActiveServerIndex(item.index);ensureRchNws();if(callback)callback();Lampa.Controller.toggle(enabled);Lampa.Activity.replace()}}})}function openServerMenu(callback){openServerSelect(callback)}function checkServerAvailability(serverUrl,callback){var baseUrl=serverUrl.replace(/\/+$/,"");var checkUrl=baseUrl+"/lite/events?life=true&id=76600&imdb_id=tt1630029&kinopoisk_id=505898&serial=0&title=Avatar: The Way of Water&original_title=Avatar: The Way of Water&original_language=en&year=2022&source=tmdb&clarification=0&similar=false&rchtype=&uid=guest&device_id=";var attempts=0;var maxAttempts=15;var memkey="";function checkForMirage(sources){for(var i=0;i<sources.length;i++){var src=sources[i];var name=(src.balanser||src.name||"").toLowerCase();if(name.indexOf("mirage")!==-1||name.indexOf("alloha")!==-1){return true}}return false}function poll(){var net=new Lampa.Reguest;net.timeout(5000);var url=memkey?baseUrl+"/lifeevents?memkey="+memkey+"&id=76600&imdb_id=tt1630029&kinopoisk_id=505898&serial=0&title=Avatar: The Way of Water&original_title=Avatar: The Way of Water&original_language=en&year=2022&source=tmdb&clarification=0&similar=false&rchtype=&uid=guest&device_id=":checkUrl;net.silent(url,function(json){var sources=json&&json.online?json.online:Lampa.Arrays.isArray(json)?json:[];if(json&&json.accsdb){callback(false);return}if(json&&json.memkey){memkey=json.memkey}if(json&&json.ready){callback(checkForMirage(sources));return}attempts++;if(attempts>=maxAttempts){callback(checkForMirage(sources))}else{setTimeout(poll,1000)}},function(){callback(false)})}poll()}var STORAGE_KEY_PUBLIC_SERVERS_CACHE="lamponline_public_servers_cache";function getCachedPublicServers(){var cached=Lampa.Storage.get(STORAGE_KEY_PUBLIC_SERVERS_CACHE,[]);if(typeof cached==="string"){try{cached=JSON.parse(cached)}catch(e){cached=[]}}return Lampa.Arrays.isArray(cached)?cached:[]}function setCachedPublicServers(servers){Lampa.Storage.set(STORAGE_KEY_PUBLIC_SERVERS_CACHE,servers)}function showPublicServersMenu(workingServers,enabled){var items=[];var userServers=getServersList();function normalizeUrl(url){return url.replace(/^https?:\/\//,"").replace(/\/+$/,"").toLowerCase()}var normalizedUserServers=userServers.map(normalizeUrl);items.push({title:Lampa.Lang.translate("lampac_refresh_servers"),refresh:true});items.push({title:Lampa.Lang.translate("lampac_public_servers"),separator:true});workingServers.forEach(function(url){var normalizedUrl=normalizeUrl(url);var isAdded=normalizedUserServers.indexOf(normalizedUrl)!==-1;items.push({title:formatServerDisplay(url),url:url,subtitle:isAdded?Lampa.Lang.translate("lampac_server_already_added"):""})});Lampa.Select.show({title:Lampa.Lang.translate("lampac_select_public_server")+" ("+workingServers.length+")",items:items,onBack:function onBack(){Lampa.Controller.toggle(enabled)},onSelect:function onSelect(item){if(item.separator)return;if(item.refresh){Lampa.Select.close();fetchAndCheckPublicServers(enabled,true);return}if(addServer(item.url)){var servers=getServersList();setActiveServerIndex(servers.length-1);ensureRchNws();Lampa.Settings.update()}else{var servers=getServersList();var idx=servers.indexOf(item.url);if(idx!==-1){setActiveServerIndex(idx);ensureRchNws();Lampa.Settings.update()}}Lampa.Controller.toggle(enabled)}})}function fetchAndCheckPublicServers(enabled,forceRefresh){Lampa.Noty.show(Lampa.Lang.translate("lampac_loading"));var network=new Lampa.Reguest;network.timeout(10000);network.silent("https://ipavlin98.github.io/lampac-links/working_online_lampa.json",function(json){if(!Lampa.Arrays.isArray(json)||json.length===0){Lampa.Noty.show(Lampa.Lang.translate("lampac_no_servers_found"));return}var serversToCheck=[];var serverCountryMap={};json.forEach(function(server){if(server.base_url){serversToCheck.push(server.base_url);if(server.country){serverCountryMap[server.base_url]=server.country}}});if(serversToCheck.length===0){Lampa.Noty.show(Lampa.Lang.translate("lampac_no_servers_found"));return}var workingServers=[];var checked=0;var total=serversToCheck.length;var notyInterval;function updateNoty(){Lampa.Noty.show(Lampa.Lang.translate("lampac_checking_servers")+" "+checked+"/"+total)}updateNoty();notyInterval=setInterval(updateNoty,2000);serversToCheck.forEach(function(serverUrl){checkServerAvailability(serverUrl,function(isWorking){checked++;if(isWorking){workingServers.push(serverUrl);if(serverCountryMap[serverUrl]){setServerCountry(serverUrl,serverCountryMap[serverUrl])}}updateNoty();if(checked===total){clearInterval(notyInterval);if(workingServers.length===0){Lampa.Noty.show(Lampa.Lang.translate("lampac_no_working_servers"));return}setCachedPublicServers(workingServers);showPublicServersMenu(workingServers,enabled)}})})},function(e){console.error(e);Lampa.Noty.show(Lampa.Lang.translate("lampac_load_error"))})}function loadPublicServers(){var enabled=Lampa.Controller.enabled().name;var cached=getCachedPublicServers();if(cached.length>0){showPublicServersMenu(cached,enabled)}else{fetchAndCheckPublicServers(enabled,false)}}if(!window.lamponline_plugin)startPlugin();function initBalanserInFilterMenu(){if(window.lamponline_src_filter_plugin){return}window.lamponline_src_filter_plugin=true;Lampa.Controller.listener.follow("toggle",function(event){if(event.name!=="select"){return}var active=Lampa.Activity.active();if(!active||!active.component||active.component.toLowerCase()!=="lamponline"){return}var $filterTitle=$(".selectbox__title");if($filterTitle.length!==1||$filterTitle.text()!==Lampa.Lang.translate("title_filter")){return}var $sourceBtn=$(".simple-button--filter.filter--sort");if($sourceBtn.length!==1||$sourceBtn.hasClass("hide")){return}if($(".selectbox-item[data-lamponline-source]").length>0){return}var $serverBtn=$(".simple-button--filter.filter--server");var $selectBoxItem=Lampa.Template.get("selectbox_item",{title:Lampa.Lang.translate("settings_rest_source"),subtitle:$("div",$sourceBtn).text()});$selectBoxItem.attr("data-lamponline-source","true");$selectBoxItem.on("hover:enter",function(){$sourceBtn.trigger("hover:enter")});$(".selectbox-item").first().after($selectBoxItem);if($serverBtn.length===1&&!$serverBtn.hasClass("hide")){var $serverSelectBoxItem=Lampa.Template.get("selectbox_item",{title:Lampa.Lang.translate("lampac_server_short"),subtitle:$("div",$serverBtn).text()});$serverSelectBoxItem.attr("data-lamponline-server","true");$serverSelectBoxItem.on("hover:enter",function(){Lampa.Select.close();$serverBtn.trigger("hover:enter")});$(".selectbox-item").first().after($serverSelectBoxItem)}Lampa.Controller.collectionSet($("body > .selectbox").find(".scroll__body"))})}if(window.appready){initBalanserInFilterMenu()}else{Lampa.Listener.follow("app",function(event){if(event.type==="ready"){initBalanserInFilterMenu()}})}})();
